...
const Showcase = () => {
  ...
  return (
    <section
      id="showcase"
      ref={showcaseRef}
      className="relative w-full min-h-screen bg-gradient-to-b from-[#030b17] via-[#041c2f] to-[#03111e] px-6 py-24 text-white flex flex-col items-center justify-start"
    >
      <motion.h2
        className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-cyan-300 to-blue-500 bg-clip-text text-transparent mb-10"
  ...
  {tabs.map((tab) => {
    ...
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        className={`group relative px-6 py-4 rounded-xl border transition-all duration-300 ease-in-out text-center 
          ${
            isActive
